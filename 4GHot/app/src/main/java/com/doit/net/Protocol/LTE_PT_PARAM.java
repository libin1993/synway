...
//        UIEventManager.call(UIEventManager.KEY_REFRESH_DEVICE);
    ...
    public static void processNamelistQuery(LTEReceivePackage receivePackage) {
        ...
        if (splitStr.length >= 7){
            namelist.setMode(splitStr[0].split(":")[1]);
            namelist.setRedirectConfig(splitStr[1].split(":")[1]);
            namelist.setNamelistReject(splitStr[2].split(":")[1]);
            namelist.setNamelistRedirect(splitStr[3].split(":")[1]);
            namelist.setNamelistBlock(splitStr[4].split(":")[1]);
            namelist.setNamelistRelease(splitStr[5].split(":")[1]);
            namelist.setNamelistRestAciton(splitStr[6].split(":")[1]);
        }
        ...
        if (locRpt.contains("#")){  /* 一条上报中含有多个IMSI */
            if (CacheManager.currentWorkMode.equals("0")){
                if (CacheManager.getLocState()){
                    ...
                    for (int i = 0; i < splitStr.length; i++) {
                        SRSP = splitStr[i].split(":")[1];
                        if (SRSP.equals(""))
                            continue;

                        String tmpImsi = splitStr[i].split(":")[0];
                        if (tmpImsi.equals(CacheManager.getCurrentLoction().getImsi())) {
