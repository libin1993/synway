...
public class LocationFragment extends BaseFragment implements View.OnClickListener, IHandlerFinish, EventAdapter.EventCall {
    ...
    private void initView() {
        ...
        cbGainSwitch.setOnCheckedChangeListener(gainSwitchListner);
            ...
            public void run() {
                if (CacheManager.getLocState()){
                    if ((int)(System.currentTimeMillis() - lastLocRptTime) > UPDATE_ARFCN_TIMEOUT){
                        UtilBaseLog.printLog("定位上报超时，尝试重新设置频点和功率... ...");
    ...
    void addLocation(String imsi){
        UtilBaseLog.printLog("##########  addLocation:" + imsi + "  ###########");
    ...
    private void stopLoc() {
        UtilBaseLog.printLog("call stopLoc() in locationFragment... ...");
        if (CacheManager.getLocState()){
    ...
    CompoundButton.OnCheckedChangeListener gainSwitchListner = new CompoundButton.OnCheckedChangeListener() {
        @Override
        public void onCheckedChanged(CompoundButton view, boolean isChecked) {
            if (!view.isPressed())
                return;
