...
public class DeviceParamActivity extends AppCompatActivity  implements IHandlerFinish {
    ...
    public void setPowerLevel(int powerLevel){
        ...
        if ((powerLevel < POWER_LEVEL_HIGH) && (powerLevel > POWER_LEVEL_LOW)){
            UtilBaseLog.printLog("set power level error param.");
            return;
        }
        ...
        for(LteChannelCfg channel:CacheManager.getChannels()){
            tmpGa = String.valueOf(Integer.valueOf(channel.getPMax())+powerAtt);
            gaConfig = tmpGa +",";
            gaConfig += gaConfig;
            gaConfig += tmpGa;
            UtilBaseLog.printLog("下发功率设置："+"IDX:"+channel.getIdx()+"@"+"PA:"+gaConfig);
...
//				.setOnClickListener(clickListener)
                .setOnItemClickListener(new OnItemClickListener() {
                    @Override public void onItemClick(DialogPlus dialog, Object item, View view, int position) {
                        ...
                        dialog.dismiss();
                        ToastUtils.showMessageLong(GameApplication.appContext,"切换Band命令已下发，请等待生效");
    ...
    private Handler mHandler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            if(msg.what == UPDATE_VIEW){
                UtilBaseLog.printLog("设备参数页面已更新。");
                refreshViews();
